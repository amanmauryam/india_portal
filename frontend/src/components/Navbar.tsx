    ...
    <>
      <header className="sticky top-0 z-40 w-full border-b border-slate-200/80 bg-white/80 backdrop-blur-md dark:border-slate-800/80 dark:bg-slate-950/80">
        <div className="mx-auto flex h-14 sm:h-16 max-w-7xl items-center justify-between px-3 sm:px-6 lg:px-8">
          {/* Logo */}
          <Link href="/" className="flex items-center gap-1.5 sm:gap-2 transition-opacity hover:opacity-90 shrink-0">
            <div className="flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-lg sm:rounded-xl bg-gradient-to-tr from-amber-500 via-white to-emerald-600 p-0.5 shadow-sm ring-1 ring-slate-200 dark:ring-slate-800">
              <div className="flex h-full w-full items-center justify-center rounded-[8px] sm:rounded-[10px] bg-slate-900 text-white font-bold text-sm sm:text-lg">
                🇮🇳
              </div>
            </div>
            <div>
              ...
              <p className="hidden sm:block text-[10px] font-medium text-slate-500 dark:text-slate-400 leading-tight">Hyperlocal Utility Portal</p>
            </div>
          </Link>
          ...
          <div className="flex items-center gap-1 sm:gap-2">
            <button
              ...
              className="flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50/50 px-1.5 py-1 text-left text-[11px] text-slate-500 hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-900 dark:hover:bg-slate-800 sm:px-3 sm:py-1.5 sm:text-xs md:w-64"
            >
              <Search className="h-3 w-3 sm:h-3.5 sm:w-3.5 text-slate-400 shrink-0" />
              <span className="hidden sm:inline">Search states, districts...</span>
              <span className="sm:hidden truncate max-w-[60px]">Search</span>
            </button>

            <button
              onClick={() => setIsOpen(!isOpen)}
              className="rounded-lg p-1.5 text-slate-600 hover:bg-slate-100 focus:outline-none md:hidden dark:text-slate-300 dark:hover:bg-slate-800"
            >
